...
from scripts.predict_off_state_duration_using_baseline_activity import (
    _standardize_cells_then_average,
)
from scripts.prepare_data_for_mixedlm import _group_features
...
class MeanCellActivityTest(unittest.TestCase):
    ...
    def test_fixedlm_filters_weights_with_unusable_cells(self):
        rates = np.asarray(
            [
                [1.0, 5.0, 2.0],
                [2.0, 5.0, 4.0],
        ...
        actual, n_cells, status = _standardize_cells_then_average(
            rates,
            np.asarray([0, 1, 2]),
            np.asarray([1.0, 100.0, 3.0]),
        )
        first = (rates[:, 0] - rates[:, 0].mean()) / rates[:, 0].std()
        third = (rates[:, 2] - rates[:, 2].mean()) / rates[:, 2].std()

        np.testing.assert_allclose(actual, (first + 3.0 * third) / 4.0)
