...
def _fit_cv_model(train, spec, max_iterations):
    """Keep failures local to a model while limiting numerical worker threads."""
    try:
        with threadpool_limits(limits=1):
            result, messages = _fit_model(train, spec, max_iterations)
...
def run_trial_holdout_cv(
    ...
    with log_path.open("w") as log:
        ...
        for repeat, split in enumerate(splits):
            ...
            for threshold, threshold_requests in requests_by_threshold.items():
                ...
                with worker_context(config.n_jobs):
                    fits = Parallel()(
                        delayed(_fit_cv_model)(train, request.spec, config.max_iterations)
                        for request in threshold_requests
                    )
                for request, (fit_result, fit_warnings, fit_error) in zip(threshold_requests, fits):
                    spec = request.spec
                    ...
                    try:
                        ...
                        result, warning_messages = fit_result, fit_warnings
        ...
        required_metrics = ['train_log_likelihood', 'train_df_modelwc'] + [
            f'{kind}_{metric}' for kind in ('fixed', 'conditional')
            for metric in ('rmse_ms', 'mae_ms', 'r2', 'session_centered_r2', 'pearson_r')]
