...
def score_probabilities(probabilities, labels, predicted_labels=None, *, context='Scores'):
    ...
    Samples represent the observed estimate or individual null shuffles. NaN denotes an unavailable
...
def evaluate_session(source):
    """Score the single observed estimate and every null shuffle."""
...
def summary_row(result):
    row = {key: result[key] for key in (
        'session', 'cue', 'num_trials', 'num_time_bins', 'num_null_shuffles'
    )}
