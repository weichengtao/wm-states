...
class Config:
    ...
    min_cell_per_group: int = 1 # a good session has at least one group with this many cells
    min_trials_good_session: int = 320 # a good session has at least this many total trials
...
def main(config: Config):
    ...
    good_sessions = {}
    for selection in selection_outs:
        if selection['max_num_cells_per_group'] < config.min_cell_per_group or selection['num_trials'] < config.min_trials_good_session:
            continue
        session = selection['session']
        if session in good_sessions:
            raise ValueError(f'Duplicate full-session selection for {session}.')
        cue = preferred_cue_from_cells(selection['cell_properties']['mean_pref_test'])
        if cue is not None:
            good_sessions[session] = {'preferred_cue': cue, 'selection': selection}
    ...
    def process_session(session: str):
        ...
        props = selection['cell_properties']
        pref_mask = np.asarray(props['mean_pref_test']) == pref_cue
        cell_idx = np.asarray(props['cell_idx'])[pref_mask]
        mean_pev = np.asarray(props['mean_pev_test'])[pref_mask]
        if cell_idx.size < config.min_cell_per_group:
            print(
                f'  Skipping: preferred-cue cell pool has {cell_idx.size} cells '
                f'(< {config.min_cell_per_group})'
            )
            return None

        if selection['num_trials'] != cue_labels.size:
            raise ValueError(f'{session}: selection and data have different trial counts.')
        trial_mask = trial_boo_correct & (cue_labels == pref_cue)
        trial_idx_pref = np.nonzero(trial_mask)[0]
        if trial_idx_pref.size == 0:
            print('  Skipping: no correct preferred-cue trials')
            return None
    ...
    with worker_context(config.n_jobs):
        results = Parallel(verbose=5)(delayed(process_session)(session) for session in sessions)
    results = [res for res in results if res is not None]
    with open(cache_dir / 'population_isi.pkl', 'ab') as f:
