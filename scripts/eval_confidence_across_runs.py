    ...
    for path in paths:
        ...
        if not cache_file.exists():
            raise FileNotFoundError(
                f'Missing {cache_file}. Run scripts/eval_confidence.py '
            ...
            for kind in ('observed', 'null'):
                ...
                if scores is not None and any(
                    ...
                    raise ValueError(f'{cache_file}, session {session}: missing metrics. '
                                     f'Rerun scripts/eval_confidence.py --cache-dir {path}.')
...
def score_curve(result, kind, metric, num_bins, null_shading='percentiles'):
    """Return repeat-0 observed scores or null means with optional bands."""
    if null_shading not in ('confidence_intervals', 'percentiles', 'none'):
        raise ValueError(f'Unknown null shading mode: {null_shading}')
    scores = result.get(kind)
    if kind == 'observed':
        if result.get('observed_repeat_idx') == 0:
            return np.asarray(scores[f'{metric}_by_time_bin'], dtype=float), None
        # Older evaluation caches retain individual repeat scores: use only 0.
        legacy = result.get('observed_repeats')
        if legacy is not None:
            samples = np.asarray(legacy[f'{metric}_by_time_bin_and_sample'], dtype=float)
            if samples.ndim == 2 and samples.shape[0] == num_bins and samples.shape[1] > 0:
                return samples[:, 0], None
        raise ValueError('Observed repeat 0 is unavailable. Rerun scripts/eval_confidence.py '
                         'for this cache directory.')
    if scores is None:
