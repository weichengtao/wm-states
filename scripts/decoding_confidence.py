...
def main(config: Config):
    ...
    for session, parts in partitions_by_session.items():
        total_trials = total_unique_trials(parts)
        if total_trials < config.min_trials_good_session:
            continue
        partition_pref = []
        for p in parts:
    ...
    for idx_session, session in enumerate(sessions, start=1):
        ...
        cell_idx_set = set()
        for p in session_info['partitions']:
        ...
        print(
            ...
            f'{cell_idx.size} cells, {selected_trial_idx.size} trials '
        ...
        # Decode each preferred-cue trial in parallel.
        decoded = Parallel(n_jobs=config.n_jobs, verbose=10)(
            delayed(decode_one_trial)(
                idx,
                binned_rates,
                labels_sel,
                config.seed,
        ...
        decode_confidence = np.stack([d[0] for d in decoded], axis=0)
        decode_confidence_null = None
        if config.n_decode_shuffle > 0:
            decode_confidence_null = np.stack([d[1] for d in decoded], axis=0)

        trial_idx_pref = selected_trial_idx[test_sel_indices]
        ...
        results.append({
            ...
            'num_trials': int(trial_idx_pref.size),
        })
